...
- **AI Chat**: Any OpenRouter model (default: google/gemini-2.5-flash:free) via Cloudflare Worker proxy with SSE streaming. Full model picker with search, free/paid sections, and recommended badges.
...
|-------|----------|---------|
| `POST /chat` | `openrouter.ai/api/v1/chat/completions` | Vision + streaming chat (via OpenRouter) |
| `GET  /models` | `openrouter.ai/api/v1/models` | Available model list for the model picker |
| `POST /transcribe-token` | `streaming.assemblyai.com/v3/token` | Fetches a short-lived (480s) AssemblyAI websocket token |

Worker secrets: `OPENROUTER_API_KEY`, `ASSEMBLYAI_API_KEY`
...
| `OpenRouterModelFetcher.swift` | ~150 | Fetches and parses the OpenRouter model list via the Worker proxy. Provides `OpenRouterModel` structs, recommended badge mapping, and caching. |
...
| `worker/src/index.ts` | ~132 | Cloudflare Worker proxy. Three routes: `/chat` (OpenRouter chat), `/models` (OpenRouter model list), `/transcribe-token` (AssemblyAI temp token). |
