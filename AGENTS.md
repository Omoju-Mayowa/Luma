...
| `Agent/AgentSession.swift` | ~220 | Core agent session model. ObservableObject with id, title, accentTheme, status, transcript entries, response card. Binds to AgentRuntime, handles submit/stop/warmUp, auto-generates titles via lightweight API call. |
...
| `Agent/ClaudeAPIAgentRuntime.swift` | ~300 | Tool-use loop fallback runtime via OpenRouter API. 7 tools (bash, screenshot, click, type, key_press, open_app, wait). Max 50 iterations safety limit. |
...
| `Agent/LumaAgentDockWindowManager.swift` | ~150 | Floating bottom-of-screen dock showing active agent sessions as circular icons with accent color gradients and status dots. |
